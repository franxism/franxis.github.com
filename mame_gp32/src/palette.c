...
{
	game_palette = gm_zi_malloc(3 * Machine->drv->total_colors * sizeof(unsigned char));
	palette_map = gm_zi_malloc(Machine->drv->total_colors * sizeof(unsigned short));
	if (Machine->drv->color_table_len)
	{
		game_colortable = gm_zi_malloc(Machine->drv->color_table_len * sizeof(unsigned short));
		Machine->colortable = gm_zi_malloc(Machine->drv->color_table_len * sizeof(unsigned short));
	}
	else game_colortable = Machine->colortable = 0;

	/* ASG 980209 - allocate space for the pens */
	shrinked_pens = gm_zi_malloc(STATIC_MAX_PENS * sizeof (short));

	Machine->pens = gm_zi_malloc(Machine->drv->total_colors * sizeof (short));
			...
			Machine->drv->total_colors > DYNAMIC_MAX_PENS)
	{
		...
		palette_used_colors = gm_zi_malloc(3 * Machine->drv->total_colors * sizeof(unsigned char));
		...
		gm_memset(palette_used_colors,PALETTE_COLOR_USED,Machine->drv->total_colors * sizeof(unsigned char));
		gm_memset(old_used_colors,PALETTE_COLOR_UNUSED,Machine->drv->total_colors * sizeof(unsigned char));
	}
...
{
	gm_free(palette_used_colors);
	palette_used_colors = old_used_colors = just_remapped = 0;
	gm_free(game_palette);
	game_palette = 0;
	gm_free(palette_map);
	palette_map = 0;
	gm_free(game_colortable);
	game_colortable = 0;
	gm_free(Machine->colortable);
	Machine->colortable = 0;
	gm_free(shrinked_pens);
	shrinked_pens = 0;
	gm_free(Machine->pens);
	Machine->pens = 0;
}
...
void palette_init(void)
{
	...
	{
		gm_memcpy(game_palette,Machine->gamedrv->palette,3 * Machine->drv->total_colors * sizeof(unsigned char));
		if (Machine->gamedrv->colortable)
			gm_memcpy(game_colortable,Machine->gamedrv->colortable,Machine->drv->color_table_len * sizeof(unsigned short));
	}
	else
	{
...
static void build_rgb_to_pen(void)
{
	...
	for(a=0;a<64;a++) {
		for(b=0;b<64;b++) {
			for(c=0;c<64;c++)	
				rgb6_to_pen[a][b][c]=DYNAMIC_MAX_PENS;
		}		
	}
	/*gm_memset(rgb6_to_pen,DYNAMIC_MAX_PENS,sizeof(rgb6_to_pen));*/
	...
	for(a=0;a<4;a++) {
		for(b=0;b<4;b++) {
			for(c=0;c<4;c++)	
				rgb2_to_pen[a][b][c]=BLACK_PEN;
		}		
	}
	/*gm_memset(rgb2_to_pen,BLACK_PEN,sizeof(rgb2_to_pen));*/

	for(a=0;a<8;a++) {
		for(b=0;b<8;b++) {
			for(c=0;c<8;c++)	
				rgb3_to_pen[a][b][c]=DYNAMIC_MAX_PENS;
		}		
	}
	/*gm_memset(rgb3_to_pen,DYNAMIC_MAX_PENS,sizeof(rgb3_to_pen));*/
	
	for(a=0;a<16;a++) {
		for(b=0;b<16;b++) {
			for(c=0;c<16;c++)	
				rgb4_to_pen[a][b][c]=DYNAMIC_MAX_PENS;
		}		
	}
	/*gm_memset(rgb4_to_pen,DYNAMIC_MAX_PENS,sizeof(rgb4_to_pen));*/
	
	for(a=0;a<32;a++) {
		for(b=0;b<32;b++) {
			for(c=0;c<32;c++)	
				rgb5_to_pen[a][b][c]=DYNAMIC_MAX_PENS;
		}		
	}
	/*gm_memset(rgb5_to_pen,DYNAMIC_MAX_PENS,sizeof(rgb5_to_pen));*/
...
const unsigned char *palette_recalc(void)
{
	...
	/* FRANXIS 30-01-2005 */
	/*gm_memset(just_remapped,0,Machine->drv->total_colors * sizeof(unsigned char));*/
	for(i=0;i<(Machine->drv->total_colors * sizeof(unsigned char));i++)
		just_remapped[i]=0;
	/* FRANXIS 30-01-2005 */
