...
{
	unsigned char *temp = gm_zi_malloc (65536 * 2);
	int i, offs;
	...
	if (temp)
	{
		for (offs = 0x120000; offs < 0x220000; offs += 0x20000)
		{
			...
			gm_memcpy (temp, base, 65536 * 2);
			for (i = 0; i < 16; i++)
			{
				gm_memcpy (&base[0x00000 + i * 0x800], &temp[0x0000 + i * 0x2000], 0x800);
				gm_memcpy (&base[0x10000 + i * 0x800], &temp[0x0800 + i * 0x2000], 0x800);
				gm_memcpy (&base[0x08000 + i * 0x800], &temp[0x1000 + i * 0x2000], 0x800);
				gm_memcpy (&base[0x18000 + i * 0x800], &temp[0x1800 + i * 0x2000], 0x800);
			}
		}
