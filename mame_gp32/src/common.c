...
int readroms(void)
{
	...
	while (romp->name || romp->offset || romp->length)
	{
		...
		if (romp->name || romp->length)
		{
			...
			gp32_text_log("Error in RomModule definition: expecting ROM_REGION");
			goto getout;
		}

		region_size = romp->offset & ~ROMFLAG_MASK;
		if ((Machine->memory_region[region] = gm_zi_malloc(region_size)) == 0)
		{
			/*printf("readroms():  Unable to allocate %d bytes of RAM\n",region_size);*/
			gp32_text_log("Unable to allocate RAM");
			goto getout;
		}
		Machine->memory_region_length[region] = region_size;

		/* some games (i.e. Pleiades) want the memory clear on startup */
		gm_memset(Machine->memory_region[region],0,region_size);
		...
		while (romp->length)
		{
			...
			if (romp->name == 0)
			{
				...
				gp32_text_log("Error in RomModule definition: ROM_CONTINUE not preceded by ROM_LOAD");
				goto getout;
			}
			else if (romp->name == (char *)-1)
			{
				/*printf("Error in RomModule definition: ROM_RELOAD not preceded by ROM_LOAD\n");*/
				gp32_text_log("Error in RomModule definition: ROM_RELOAD not preceded by ROM_LOAD\n");
				goto getout;
			}

			name = romp->name;
			gp32_text_log((char *)name);
			...
			{
				/* Unable to open ROM */
				gp32_text_log("Unable to open ROM");
				goto printromlist;
			}
			...
			do
			{
				...
				if (romp->offset + length > region_size)
				{
					...
					gp32_text_log("Error in RomModule definition: out of memory region space\n");
					osd_fclose(f);
					goto getout;
				}
				...
				if (romp->length & ROMFLAG_ALTERNATE)
				{
					...
					temp = gm_zi_malloc (length);

					if (!temp)
					{
						/*printf("Out of memory reading ROM %s\n",name);*/
						gp32_text_log("Out of memory reading ROMs");
						osd_fclose(f);
						goto getout;
					}
					...
					if (osd_fread(f,temp,length) != length)
					{
						...
						gp32_text_log("Unable to read ROM");
						gm_free(temp);
						osd_fclose(f);
						goto printromlist;
					}
			...
			if (explength != osd_fsize (f))
			{
				...
				gp32_text_log("Length mismatch on ROM");
				lengthwarning++;
			}
			...
			if (expchecksum != osd_fcrc (f))
			{
				...
				if (checksumwarning == 0)
					/*printf ("The checksum of some ROMs does not match that of the ones MAME was tested with.\n"
							...
							"Name         Expected  Found\n");*/
					gp32_text_log("Wrong checksum of ROM");
				checksumwarning++;
				/*if (expchecksum)*/
...
/* if first samplename is *dir, looks for samples into "basename" first, then "dir" */
{
	...
	if ((samples = gm_zi_malloc(sizeof(struct GameSamples) + (i-1)*sizeof(struct GameSample))) == 0)
		return 0;
...
struct GfxElement *decodegfx(const unsigned char *src,const struct GfxLayout *gl)
{
	...
	if ((gfx = gm_zi_malloc(sizeof(struct GfxElement))) == 0) {
		return 0;
	}
	...
	if (gfx->color_granularity <= 32)	/* can't handle more than 32 pens */
		gfx->pen_usage = gm_zi_malloc(gfx->total_elements * sizeof(int));
			...
			case TRANSPARENCY_NONE:		/* do a verbatim copy (faster) */
				...
				else		/* normal */
				{
					for (y = sy;y <= ey;y++)
					{
						bm = dest->line[y] + sx;
						sd = gfx->gfxdata->line[start] + (sx-ox);
						/* gm_memcpy(bm,sd,ex-sx+1); FRANXIS 30-01-2005*/
						for (ifr=0;ifr<ex-sx+1;ifr++)
							bm[ifr]=sd[ifr];
						/* FRANXIS 30-01-2005 */
						start+=dy;
					}
...
void fillbitmap(struct osd_bitmap *dest,int pen,const struct rectangle *clip)
{
	...
	/* FRANXIS 30-01-2005 */
	/*for (y = sy;y <= ey;y++)
		gm_memset(&dest->line[y][sx],pen,ex-sx+1);*/
	for (y = sy;y <= ey;y++) {
		for (i=0;i<ex-sx+1;i++)
			dest->line[y][sx+i] = pen;
