...
static int memory_allocate_ext (void)
{
	...
	for (cpu = 0; cpu < cpu_gettotalcpu (); cpu++)
	{
		...
		while (1)
		{
			...
			ext->end = end - 1;
			ext->region = region;
			ext->data = gm_zi_malloc (end - lowest);
			...
			/* reset the memory */
			gm_memset (ext->data, 0, end - lowest);
			size = ext->end + 1;
			ext++;
		}
	}
...
int initmemoryhandlers(void)
{
	...
	for( cpu = 0 ; cpu < cpu_gettotalcpu() ; cpu++ )
	{
		...
		if( (cur_mr_element[cpu] = (MHELE *)gm_zi_malloc(sizeof(MHELE)<<abits1)) == 0 )
		{
			shutdownmemoryhandler();
			return 0;
		}

		if( (cur_mw_element[cpu] = (MHELE *)gm_zi_malloc(sizeof(MHELE)<<abits1)) == 0 )
		{
			shutdownmemoryhandler();
			return 0;
		}
...
void shutdownmemoryhandler(void)
{
	...
	for( cpu = 0 ; cpu < MAX_CPU ; cpu++ )
	{
		...
		{
			gm_free( cur_mr_element[cpu] );
			cur_mr_element[cpu] = 0;
		}
		if( cur_mw_element[cpu] != 0 )
		{
			gm_free( cur_mw_element[cpu] );
	...
	for (ext = ext_memory; ext->data; ext++)
		gm_free (ext->data);
	gm_memset (ext_memory, 0, sizeof (ext_memory));
}
