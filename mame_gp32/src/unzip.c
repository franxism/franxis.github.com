...
static int ecd_read(ZIP* zip) {
	...
	while (1) {
		...
		if (gp32_fseek(zip->fp, zip->length - buf_length, SEEK_SET) != 0) {
			return -1;
		}

		/* allocate buffer */
		buf = (char*)gm_zi_malloc( buf_length );
		if (!buf) {
			return -1;
		}

		if (gp32_fread( buf, buf_length, 1, zip->fp ) < 1 /*!=1 FRANXIS 17-01-2005 */) {
			gm_free(buf);
			return -1;
		}
		...
		if (ecd_find_sig(buf, buf_length, &offset)) {
			...
			zip->ecd = (char*)gm_zi_malloc( zip->ecd_length );
			if (!zip->ecd) {
				gm_free(buf);
				return -1;
			}

			gm_memcpy(zip->ecd, buf + offset, zip->ecd_length);
...
ZIP* openzip(const char* zipfile) {
	/* allocate */
	ZIP* zip = (ZIP*)gm_zi_malloc( sizeof(ZIP) );
	if (!zip) {
		return 0;
	}

	/* open */
	zip->fp = gp32_fopen(zipfile, "rb");
	if (!zip->fp) {
	...
	/* go to end */
	if (gp32_fseek(zip->fp, 0L, SEEK_END) != 0) {
	...
	if (gp32_fseek(zip->fp, zip->offset_to_start_of_cent_dir, SEEK_SET)!=0) {
	...
	zip->cd = (char*)gm_zi_malloc( zip->size_of_cent_dir );
	if (!zip->cd) {
		gm_free(zip->ecd);
		gp32_fclose(zip->fp);
		gm_free(zip);
	...
	if (gp32_fread(zip->cd, zip->size_of_cent_dir, 1, zip->fp) < 1 /*!=1 FRANXIS 17-01-2005 */) {
	...
	/* file name */
	zip->zip = (char*)gm_zi_malloc(strlen(zipfile)+1);
	if (!zip->zip) {
		gm_free(zip->cd);
		gm_free(zip->ecd);
		gp32_fclose(zip->fp);
...
struct zipent* readzip(ZIP* zip) {
	...
	/* copy filename */
	gm_free(zip->ent.name);
	zip->ent.name = (char*)gm_zi_malloc(zip->ent.filename_length + 1);
	gm_memcpy(zip->ent.name, zip->cd+zip->cd_pos+ZIPCFN, zip->ent.filename_length);
...
int seekcompresszip(ZIP* zip, struct zipent* ent) {
	...
	if (gp32_fseek(zip->fp, ent->offset_lcl_hdr_frm_frst_disk, SEEK_SET)!=0) {
		errormsg ("Seeking to header", ERROR_CORRUPT, zip->zip);
		return -1;
	}

	if (gp32_fread(buf, ZIPNAME, 1, zip->fp) < 1 /*!=1 FRANXIS 17-01-2005 */) {
...

	{
		...
		if (gp32_fseek(zip->fp, offset, SEEK_SET) != 0) {
...
int readcompresszip(ZIP* zip, struct zipent* ent, char* data) {
	...
	if (gp32_fread(data, ent->compressed_size, 1, zip->fp) < 1 /*!=1 FRANXIS 17-01-2005 */) {
...
int /* error */ load_zipped_file (const char* zipfile, const char* filename, unsigned char** buf, unsigned int* length) {
	...
	while (ent) {
				...
				!strcmp(crc, filename))
		{
			...
			*buf = (unsigned char*)gm_zi_malloc( *length );
			if (!*buf) {
				cache_closezip(zip);
				return -1;
			}
