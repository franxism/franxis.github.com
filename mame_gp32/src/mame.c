...
int init_machine(void)
{
	if (gamedrv->input_ports)
	{
		...
		do
		{
			gm_memcpy(to,from,sizeof(struct InputPort));
...
int run_machine(void)
{
	...
	if (vh_open() == 0)
	{
		if (drv->vh_start == 0 || (*drv->vh_start)() == 0)      /* start the video hardware */
		{
			...
			else gp32_text_log("Unable to start audio emulation");
		}
		else gp32_text_log("Unable to start video emulation");

		vh_close();
	}
	else gp32_text_log("Unable to initialize display");
