...
int ADPCM_sh_start (struct ADPCMinterface *intf)
{
	...
	if (intf->init)
	{
		sample_list = gm_zi_malloc (257 * sizeof (struct ADPCMsample));
		if (!sample_list)
			return 1;
		gm_memset (sample_list, 0, 257 * sizeof (struct ADPCMsample));
	...
	gm_memset (adpcm, 0, sizeof (adpcm));
	for (i = 0; i < intf->num; i++)
	{
		...
		adpcm[i].buffer = gm_zi_malloc (buffer_len * Machine->sample_bits / 8);
		if (!adpcm[i].buffer)
			return 1;
	}
...
void ADPCM_sh_stop (void)
{
	...
	{
		if (adpcm[i].stream)
			gm_free (adpcm[i].stream);
		if (adpcm[i].buffer)
			gm_free (adpcm[i].buffer);
	}
}
...
static void ADPCM_update (struct ADPCMVoice *voice, int finalpos)
{
	...
	if (left > 0)
	{
		...
		{
			if (Machine->sample_bits == 16)
				gm_memset ((short *)voice->buffer + voice->bufpos, 0, left * 2);
			else
				gm_memset ((unsigned char *)voice->buffer + voice->bufpos, AUDIO_CONV (0), left);
		}
...
int MSM5205_sh_start (struct MSM5205interface *intf)
{
	...
	if (!result)
	{
		...
		for (i = 0; i < intf->num; i++)
		{
			...
			voice->stream = gm_zi_malloc (stream_size);
			if (!voice->stream)
				return 1;
