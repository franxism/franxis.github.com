...
int Pokey_sound_init (int freq17, int playback_freq, int volume, int num_pokeys, int use_clip)
{
	...
	poly17 = gm_zi_malloc (POLY17_SIZE+1);
	if (!poly17) return 1;

    /* HJB 980706 - random numbers are the upper 8 bits of poly 17 */
	rand17 = gm_zi_malloc (POLY17_SIZE+1);
	if (!rand17) return 1;
...
static char *audc2str(int val)
{
	...
	if (val & NOTPOLY5) {
		if (val & PURE) {
			...
			gm_strcpy(buff,"poly4");
        } else {
			gm_strcpy(buff,"poly9/17");
		}
	} else {
		if (val & PURE) {
			gm_strcpy(buff,"poly5");
		} else if (val & POLY4) {
			gm_strcpy(buff,"poly4+poly5");
        } else {
			gm_strcpy(buff,"poly9/17+poly5");
        }
    }
	return buff;
}
...
static char *audctl2str(int val)
{
	static char buff[80];
	if (val & POLY9) {
		gm_strcpy(buff,"poly9");
	} else {
		gm_strcpy(buff,"poly17");
    }
	if (val & CH1_179) {
		gm_strcpy(buff,"+ch1hi");
    }
	if (val & CH3_179) {
		gm_strcpy(buff,"+ch3hi");
    }
	if (val & CH1_CH2) {
		gm_strcpy(buff,"+ch1/2");
    }
	if (val & CH3_CH4) {
		gm_strcpy(buff,"+ch3/4");
    }
	if (val & CH1_FILTER) {
		gm_strcpy(buff,"+ch1filter");
    }
	if (val & CH2_FILTER) {
		gm_strcpy(buff,"+ch2filter");
    }
	if (val & CLOCK_15) {
		gm_strcpy(buff,"+clk15");
    }
    return buff;
}
#endif
...
{
	if (rand17)	gm_free (rand17);
	rand17 = NULL;
	if (poly17)	gm_free (poly17);
    poly17 = NULL;
}
