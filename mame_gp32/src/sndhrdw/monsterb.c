...
static int make_mixer_table (int gain)
{
	...
	/* allocate memory */
	mixer_table = gm_zi_malloc (256 * NUM_VOICES);
	if (!mixer_table)
		return 1;
...
int TMS3617_sh_start(void)
{
	...
	if ((output_buffer = gm_zi_malloc(buffer_len)) == 0)
		return 1;

	if ((mixer_buffer = gm_zi_malloc(sizeof(short) * buffer_len)) == 0)
	{
		gm_free (output_buffer);
		return 1;
	}

	if (make_mixer_table (intf->gain))
	{
		gm_free (mixer_buffer);
		gm_free (output_buffer);
		return 1;
	}
