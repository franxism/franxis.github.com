...
static int FMInitTable( void )
{
	...
	TL_TABLE = gm_zi_malloc(TL_MAX*2*sizeof(int));
               ...
               FM_TIMERHANDLER TimerHandler,FM_IRQHANDLER IRQHandler)
{
	...
	if( (FM2203 = (YM2203 *)gm_zi_malloc(sizeof(YM2203) * FMNumChips))==NULL)
		return (-1);
	/* clear */
	gm_memset(FM2203,0,sizeof(YM2203) * FMNumChips);
...

{
	...
	if( (FM2608 = (YM2608 *)gm_zi_malloc(sizeof(YM2608) * FMNumChips))==NULL)
		return (-1);
	/* clear */
	gm_memset(FM2608,0,sizeof(YM2608) * FMNumChips);
	/* allocate total level table (128kb space) */
	if( !FMInitTable() )
	{
		gm_free( FM2608 );
...

{
	...
	/* allocate extend state space */
	if( (FM2610 = (YM2610 *)gm_zi_malloc(sizeof(YM2610) * FMNumChips))==NULL)
		return (-1);
	/* clear */
	gm_memset(FM2610,0,sizeof(YM2610) * FMNumChips);
	/* allocate total level table (128kb space) */
	if( !FMInitTable() )
	{
		gm_free( FM2610 );
               ...
               FM_TIMERHANDLER TimerHandler,FM_IRQHANDLER IRQHandler)
{
	...
	/* allocate extend state space */
	if( (FM2612 = (YM2612 *)gm_zi_malloc(sizeof(YM2612) * FMNumChips))==NULL)
		return (-1);
	/* clear */
	gm_memset(FM2612,0,sizeof(YM2612) * FMNumChips);
	/* allocate total level table (128kb space) */
	if( !FMInitTable() )
	{
		gm_free( FM2612 );
               ...
               FM_TIMERHANDLER TimerHandler,FM_IRQHANDLER IRQHandler)
{
	...
	if( (FMOPM = (YM2151 *)gm_zi_malloc(sizeof(YM2151) * FMNumChips))==NULL)
		return (-1);
