...
static int parse_frame (void)
{
	...
	old_pitch = new_pitch;
	gm_memcpy( old_k , new_k , sizeof(old_k) );
	/* command byte check */
	cmd = VLM5030_rom[VLM5030_address];
	if( cmd & 0x01 )
	{	/* extend frame */
		new_energy = new_pitch = 0;
		gm_memset( new_k , 0 , sizeof(new_k));
		VLM5030_address++;
		if( cmd & 0x02 )
		{	/* end of speech */
			return 0;
		...
		while (size > 0)
		{
			...
			if( sample_count == 0 )
			{
				...
				if ( interp_count == 0 )
				{
					...
					gm_memcpy( current_k , old_k , sizeof(current_k) );
					...
					if (current_energy == 0)
					{
						...
						gm_memcpy( target_k , current_k , sizeof(target_k) );
					}
					else
					{
						...
						gm_memcpy( target_k , new_k , sizeof(target_k) );
					}
				}
				/* next interpolator */
...
/*	VLM5030_VCU(intf->vcu); */
	...
	if ((outbuffer = gm_zi_malloc(buffer_len)) == 0)
	{
		return 1;
	}
	gm_memset(outbuffer,0x80,buffer_len);
