...
int YM2151Init(int num, int clock, int rate,int sample_bits )
{
    ...
    YMPSG = (YM2151 *)gm_zi_malloc(sizeof(YM2151) * YMNumChips);
    if (YMPSG == NULL) return (1);

    TL_TAB = (signed int *)gm_zi_malloc(sizeof(signed int) * TL_TAB_LEN );
    if (TL_TAB == NULL) { gm_free(YMPSG); YMPSG=NULL; return (1); }
...
void YM2151ResetChip(int num)
{
	...
	for( i=0; i<32; i++){
		gm_memset(&chip->Oscils[i],'\0',sizeof(OscilRec));
