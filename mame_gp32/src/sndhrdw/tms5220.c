...
void tms5220_reset (void)
{
    ...
    gm_memset (old_k, 0, sizeof (old_k));
    gm_memset (new_k, 0, sizeof (new_k));
    gm_memset (current_k, 0, sizeof (current_k));
    gm_memset (target_k, 0, sizeof (target_k));

    /* initialize the sample generators */
    interp_count = sample_count = pitch_count = 0;
    randbit = 0;
    gm_memset (u, 0, sizeof (u));
    gm_memset (x, 0, sizeof (x));
