		...
		int param,void (*callback)(int param,void *buffer,int length))
{
	...
	gm_strcpy(stream_name[channel],name);
	...
	if ((stream_buffer[channel] = gm_zi_malloc((sample_bits/8)*stream_buffer_len[channel])) == 0)
		return -1;
		...
		int param,void (*callback)(int param,void **buffer,int length))
{
	...
	{
		gm_strcpy(stream_name[channel+i],name[i]);
		...
		if ((stream_buffer[channel+i] = gm_zi_malloc((sample_bits/8)*stream_buffer_len[channel+i])) == 0)
			return -1;
