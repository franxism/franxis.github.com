...
ym3812* ym3812_Init( int nReplayFrq, int nClock, int f16Bit )
{
	...
	pOPL = (ym3812*)gm_zi_malloc(sizeof(ym3812));
	gm_memset( pOPL, 0x00, sizeof(ym3812));
	...
	for( l=0; l<5; l++ )
	{
		...
		if( psSamples->sample[l] != 0 )
		{
			...
			pOPL->pDrum[l] = (signed char*) gm_zi_malloc( pOPL->nDrumSize[l]+1024 );
			gm_memset( pOPL->pDrum[l], 0, pOPL->nDrumSize[l]+1024 );			/* Clear buffer*/
...
ym3812* ym3812_DeInit( ym3812 *pOPL )
{
	...
	if( pOPL != NULL )
	{
		for( l=0; l<5; l++ )
		{
			if( pOPL->pDrum[l]!=NULL ) gm_free( pOPL->pDrum[l] );
		}
		gm_free( pOPL );
		pOPL = NULL;
	}
