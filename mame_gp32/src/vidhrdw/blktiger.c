...
int blktiger_vh_start(void)
{
	...
	if ((dirtybuffer2 = gm_zi_malloc(blktiger_backgroundram_size * scroll_page_count)) == 0)
	{
		generic_vh_stop();
		return 1;
	}
	gm_memset(dirtybuffer2,1,blktiger_backgroundram_size * scroll_page_count);

	if ((scroll_ram = gm_zi_malloc(blktiger_backgroundram_size * scroll_page_count)) == 0)
	{
		generic_vh_stop();
		return 1;
	}
	gm_memset(scroll_ram,0,blktiger_backgroundram_size * scroll_page_count);
