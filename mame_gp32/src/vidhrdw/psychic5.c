...
{
	if ((ps5_background_videoram = gm_zi_malloc(0x1000)) == 0)
	{
		return 1;
	}
	if ((ps5_dummy_bg_ram = gm_zi_malloc(0x1000)) == 0)
	{
		gm_free(ps5_background_videoram);
		return 1;
	}
	if ((ps5_io_ram = gm_zi_malloc(0x400)) == 0)
	{
		gm_free(ps5_background_videoram);
		gm_free(ps5_dummy_bg_ram);
		return 1;
	}
	if ((ps5_palette_ram = gm_zi_malloc(0xc00)) == 0)
	{
		gm_free(ps5_background_videoram);
		gm_free(ps5_dummy_bg_ram);
		gm_free(ps5_io_ram);
	...
	if ((ps5_foreground_videoram = gm_zi_malloc(0x1000)) == 0)
	{
		gm_free(ps5_background_videoram);
		gm_free(ps5_dummy_bg_ram);
		gm_free(ps5_io_ram);
		gm_free(ps5_palette_ram);
		return 1;
	}
        if ((bg_dirtybuffer = gm_zi_malloc(32*64)) == 0)
	{
		gm_free(ps5_background_videoram);
		gm_free(ps5_dummy_bg_ram);
		gm_free(ps5_io_ram);
		gm_free(ps5_palette_ram);
		gm_free(ps5_foreground_videoram);
        	return 1;
	}
	if ((bitmap_bg = osd_new_bitmap (Machine->drv->screen_width*4,Machine->drv->screen_height*2,Machine->scrbitmap->depth)) == 0)
	{
        ...
        gm_memset(bg_dirtybuffer,1,32*64);
	gm_memset(ps5_background_videoram, 0,0x1000);
	gm_memset(ps5_dummy_bg_ram,0,0x1000);
	gm_memset(ps5_io_ram,0,0x400);
	gm_memset(ps5_palette_ram,0,0xc00);
	gm_memset(ps5_foreground_videoram,0,0x1000);
        return 0;
}
...
void set_visible_colors(int bg_scrollx,int bg_scrolly)
{
	...
	gm_memset (colors_used, 0, sizeof (colors_used));
	for (x = 17; x >=0; x--)
	{
		for (y = 0 ; y < 18; y++)
		{
...
void psychic5_vh_screenrefresh(struct osd_bitmap *bitmap,int full_refresh)
{
	...
	} else
	{
		...
		gm_memset(bg_dirtybuffer,1,64*32);
	}
