...
{
	if ((dirtybuffer = gm_zi_malloc(firetrap_bgvideoram_size)) == 0)
	{
		return 1;
	}
	gm_memset(dirtybuffer,1,firetrap_bgvideoram_size);
	...
	if ((dirtybuffer2 = gm_zi_malloc(firetrap_bgvideoram_size)) == 0)
	{
		osd_free_bitmap(tmpbitmap);
		gm_free(dirtybuffer);
		return 1;
	}
	gm_memset(dirtybuffer2,1,firetrap_bgvideoram_size);
