...
int contra_vh_start(void)
{
	...
	if ((fgdirtybuffer = gm_zi_malloc(contra_fg_vram_size)) == 0)
	{
		osd_free_bitmap(bgbitmap);
		osd_free_bitmap(fgbitmap);
		return 1;
	}
	gm_memset(fgdirtybuffer,1,contra_fg_vram_size);

	if ((bgdirtybuffer = gm_zi_malloc(contra_bg_vram_size)) == 0)
	{
		gm_free(fgdirtybuffer);
		osd_free_bitmap(bgbitmap);
		osd_free_bitmap(fgbitmap);
		return 1;
	}
	gm_memset(bgdirtybuffer,1,contra_bg_vram_size);
