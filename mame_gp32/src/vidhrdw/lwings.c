...
int lwings_vh_start(void)
{
	...
	if (generic_vh_start() != 0)
        ...
        if ((dirtybuffer2 = gm_zi_malloc(lwings_backgroundram_size)) == 0)
	{
		generic_vh_stop();
		return 1;
	}
        gm_memset(dirtybuffer2,1,lwings_backgroundram_size);

        if ((dirtybuffer4 = gm_zi_malloc(lwings_backgroundram_size)) == 0)
	{
		generic_vh_stop();
		return 1;
	}
        gm_memset(dirtybuffer4,1,lwings_backgroundram_size);
...
#define GFX_ELEM_COLORS(gfxn) Machine->gfx[gfxn]->color_granularity
	...
	{
		gm_memset(&palette_used_colors[COLORTABLE_START(0,i)],
				PALETTE_COLOR_USED,
				GFX_ELEM_COLORS(0));
	...
	{
		gm_memset(&palette_used_colors[COLORTABLE_START(1,i)],
				PALETTE_COLOR_USED,
				GFX_ELEM_COLORS(1));
	}
	/* sprites */
	...
	{
		gm_memset(&palette_used_colors[COLORTABLE_START(2,i)],
				PALETTE_COLOR_USED,
				GFX_ELEM_COLORS(2));
	}
...
#define GFX_ELEM_COLORS(gfxn) Machine->gfx[gfxn]->color_granularity
	...
	{
		gm_memset(&palette_used_colors[COLORTABLE_START(3,i)],
				PALETTE_COLOR_USED,
				GFX_ELEM_COLORS(3));
	}
