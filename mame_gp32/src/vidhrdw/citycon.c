...
{
	if ((dirtybuffer = gm_zi_malloc(videoram_size)) == 0)
		return 1;
...
for (offs = videoram_size - 1;offs >= 0;offs--)
{
	...
	gm_memset(&palette_used_colors[256 + 16 * color],PALETTE_COLOR_USED,16);
}
for (offs = 0;offs < 256;offs++)
{
	...
	gm_memset(&palette_used_colors[512 + 4 * color + 1],PALETTE_COLOR_USED,3);
}
for (offs = spriteram_size-4;offs >= 0;offs -= 4)
{
	...
	gm_memset(&palette_used_colors[16 * color + 1],PALETTE_COLOR_USED,15);
}
