...
int goldstar_vh_start(void)
{
	...
	dirtybuffer1 = gm_zi_malloc(3 * goldstar_video_size * sizeof(unsigned char));
...
void goldstar_fa00_w(int offset,int data)
{
	...
	{
		bgcolor = (data & 0x04) >> 2;
		gm_memset(dirtybuffer1,1,goldstar_video_size);
		gm_memset(dirtybuffer2,1,goldstar_video_size);
		gm_memset(dirtybuffer3,1,goldstar_video_size);
	}
}
