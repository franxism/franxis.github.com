...
int twincobr_vh_start(void)
{
	...
	if ((videoram = gm_zi_malloc(videoram_size)) == 0)
		return 1;
	gm_memset(videoram,0,videoram_size);

	if ((twincobr_fgvideoram = gm_zi_malloc(twincobr_fgvideoram_size)) == 0)
	{
		gm_free(videoram);
		return 1;
	}
	gm_memset(twincobr_fgvideoram,0,twincobr_fgvideoram_size);

	if ((twincobr_bgvideoram = gm_zi_malloc(twincobr_bgvideoram_size)) == 0)
	{
		gm_free(twincobr_fgvideoram);
		gm_free(videoram);
		return 1;
	}
	gm_memset(twincobr_bgvideoram,0,twincobr_bgvideoram_size);

	if ((dirtybuffer = gm_zi_malloc(twincobr_bgvideoram_size / 2)) == 0)
	{
		gm_free(twincobr_bgvideoram);
		gm_free(twincobr_fgvideoram);
		gm_free(videoram);
		return 1;
	}
	gm_memset(dirtybuffer,1,twincobr_bgvideoram_size / 2);
