...
int foodf_vh_start(void)
{
	...
	if (!playfielddirty) playfielddirty = gm_zi_malloc (foodf_playfieldram_size / 2);
	if (!playfielddirty)
	{
		foodf_vh_stop ();
		return 1;
	}
	gm_memset (playfielddirty, 1, foodf_playfieldram_size / 2);
...
void foodf_vh_stop(void)
{
	...
	/* free dirty buffers */
	if (playfielddirty) gm_free (playfielddirty); playfielddirty = 0;
}
