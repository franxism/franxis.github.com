...
{
	if ((SpritesCollisionTable = gm_zi_malloc(256*256)) == 0)
		return 1;
	gm_memset(SpritesCollisionTable,255,256*256);

	if ((bg_dirtybuffer = gm_zi_malloc(1024)) == 0)
	{
		gm_free(SpritesCollisionTable);
		return 1;
	}
	gm_memset(bg_dirtybuffer,1,1024);
	if ((tx_dirtybuffer = gm_zi_malloc(1024)) == 0)
	{
		gm_free(bg_dirtybuffer);
		gm_free(SpritesCollisionTable);
		return 1;
	}
	gm_memset(tx_dirtybuffer,1,1024);
	if ((bg_ram = gm_zi_malloc(0x4000)) == 0)			/* Allocate 16k for background banked ram */
	{
		gm_free(bg_dirtybuffer);
		gm_free(tx_dirtybuffer);
		gm_free(SpritesCollisionTable);
		return 1;
	}
	gm_memset(bg_ram,0,0x4000);
...
void system8_compute_palette (void)
{
	...
	gm_memset (bg_usage, 0, sizeof (bg_usage));
	gm_memset (tx_usage, 0, sizeof (tx_usage));
	gm_memset (sp_usage, 0, sizeof (sp_usage));
	...
	{
		if (bg_usage[i])
			gm_memset (palette_used_colors + 1024 + i * 8, PALETTE_COLOR_USED, 8);
		else
			gm_memset (palette_used_colors + 1024 + i * 8, PALETTE_COLOR_UNUSED, 8);

		palette_used_colors[512 + i * 8] = PALETTE_COLOR_TRANSPARENT;
		if (tx_usage[i])
			gm_memset (palette_used_colors + 512 + i * 8 + 1, PALETTE_COLOR_USED, 7);
		else
			gm_memset (palette_used_colors + 512 + i * 8 + 1, PALETTE_COLOR_UNUSED, 7);
	}
	...
	for (i = 0; i < 32; i++)
	{
		...
		if (sp_usage[i])
			gm_memset (palette_used_colors + 0 + i * 16 + 1, PALETTE_COLOR_USED, 15);
		else
			gm_memset (palette_used_colors + 0 + i * 16 + 1, PALETTE_COLOR_UNUSED, 15);
	}
