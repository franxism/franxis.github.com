...
int starforc_vh_start(void)
{
	...
	if ((dirtybuffer2 = gm_zi_malloc(starforc_bgvideoram_size)) == 0)
	{
		generic_vh_stop();
		return 1;
	}
	gm_memset(dirtybuffer2,1,starforc_bgvideoram_size);

	if ((dirtybuffer3 = gm_zi_malloc(starforc_bgvideoram_size)) == 0)
	{
		gm_free(dirtybuffer2);
		generic_vh_stop();
		return 1;
	}
	gm_memset(dirtybuffer3,1,starforc_bgvideoram_size);
	...
	gm_memset (palette_used_colors, PALETTE_COLOR_UNUSED, 48*8);
	for (i = 0; i < 64; i++)
	{
		if (i % 8 == 0)
		{
