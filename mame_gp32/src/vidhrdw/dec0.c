...
static void dec0_update_palette(int pf23priority)
{
	...
	if (palette_recalc())
	{
		gm_memset(dec0_pf1_dirty,1,TEXTRAM_SIZE);
		gm_memset(dec0_pf2_dirty,1,TILERAM_SIZE);
		gm_memset(dec0_pf3_dirty,1,TILERAM_SIZE);
	}
}
...
int dec0_vh_start (void)
{
	...
	dec0_pf1_dirty = gm_zi_malloc(TEXTRAM_SIZE);
	dec0_pf3_dirty = gm_zi_malloc(TILERAM_SIZE);
	dec0_pf2_dirty = gm_zi_malloc(TILERAM_SIZE);
...
void dec0_vh_stop (void)
{
	...
	osd_free_bitmap(dec0_pf3_bitmap);
	osd_free_bitmap(dec0_pf1_bitmap);
	gm_free(dec0_pf3_dirty);
	gm_free(dec0_pf2_dirty);
	gm_free(dec0_pf1_dirty);
}
