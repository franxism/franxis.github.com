...
int atarisys1_vh_start(void)
{
	...
	if (!pfmapped)
		pfmapped = gm_calloc (atarigen_playfieldram_size / 2 * sizeof (int) +
		                   YDIM * sizeof (int), 1);
	if (!pfmapped)
	{
		atarisys1_vh_stop ();
	...
	/* reset the timers */
	gm_memset (int3_timer, 0, sizeof (int3_timer));
	int3_state = 0;
...
int roadblst_vh_start(void)
{
	...
	/* allocate our own pen usage table for the 6-bit background tiles */
	{
		...
		gfx = Machine->gfx[1];
		roadblst_pen_usage = gm_zi_malloc (gfx->total_elements * 4 * sizeof (short));
		if (!roadblst_pen_usage)
			return 1;
		gm_memset (roadblst_pen_usage, 0, gfx->total_elements * 4 * sizeof (short));
...
void atarisys1_update_display_list (int scanline)
{
	...
	gm_memset (spritevisit, 0, sizeof (spritevisit));
	gm_memset (timer, 0, sizeof (timer));
...
void atarisys1_vh_screenrefresh(struct osd_bitmap *bitmap,int full_refresh)
{
	...
	gm_memset (mo_map, 0, sizeof (mo_map));
	gm_memset (pf_map, 0, sizeof (pf_map));
	gm_memset (al_map, 0, sizeof (al_map));
	gm_memset (palette_used_colors, PALETTE_COLOR_UNUSED, Machine->drv->total_colors * sizeof(unsigned char));

	/* assign our special pen here */
	gm_memset (&palette_used_colors[1024], PALETTE_COLOR_TRANSPARENT, 16);
	...
	{
		if (pf_map[i])
			gm_memset (&palette_used_colors[256 + i * (16 << pf_map_shift)], PALETTE_COLOR_USED, 16 << pf_map_shift);
		if (mo_map[i])
		{
			palette_used_colors[256 + i * (16 >> mo_map_shift)] = PALETTE_COLOR_TRANSPARENT;
			gm_memset (&palette_used_colors[256 + i * (16 >> mo_map_shift) + 1], PALETTE_COLOR_USED, (16 >> mo_map_shift) - 1);
		}
	}
	for (i = 0; i < 8; i++)
		if (al_map[i])
			gm_memset (&palette_used_colors[0 + i * 4], PALETTE_COLOR_USED, 4);

	/* always remap the transluscent colors */
	gm_memset (&palette_used_colors[768], PALETTE_COLOR_USED, 16 >> mo_map_shift);
...
void roadblst_vh_screenrefresh(struct osd_bitmap *bitmap,int full_refresh)
{
	...
	for (i = 0; i < 32; i++)
	{
		...
		{
			palette_used_colors[256 + i * (16 >> mo_map_shift)] = PALETTE_COLOR_TRANSPARENT;
			gm_memset (&palette_used_colors[256 + i * 16 + 1], PALETTE_COLOR_USED, 15);
		}
	}
	for (i = 0; i < 8; i++)
		if (al_map[i])
	...
	for (y = 0; y < YDIM; y++)
	{
		...
		if (scrollx + XDIM > 0x200)
		{
			...
			gm_memcpy (&bitmap->line[y][0], &playfieldbitmap->line[dy][scrollx << shift], chunk << shift);
			gm_memcpy (&bitmap->line[y][chunk << shift], &playfieldbitmap->line[dy][0], (XDIM - chunk) << shift);
		}
		else
			gm_memcpy (&bitmap->line[y][0], &playfieldbitmap->line[dy][scrollx << shift], XDIM << shift);
	}
