...
{
	gm_memset(scroll1_dirty,1,0x1000);
	gm_memset(scroll2_dirty,1,0x1000);
}
#endif
...
int lastduel_vh_start(void)
{
	...
	scroll1_dirty=(unsigned char *)gm_zi_malloc(0x1000);
	scroll2_dirty=(unsigned char *)gm_zi_malloc(0x1000);
...
void lastduel_vh_stop(void)
{
	...
	gm_free(scroll1_dirty);
	gm_free(scroll2_dirty);
}
