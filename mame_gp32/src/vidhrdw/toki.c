...
{
	if ((frg_dirtybuffer = gm_zi_malloc (toki_foreground_videoram_size / 2)) == 0)
	{
		return 1;
	}
	if ((bg1_dirtybuffer = gm_zi_malloc (toki_background1_videoram_size / 2)) == 0)
	{
		gm_free (frg_dirtybuffer);
		return 1;
	}
	if ((bg2_dirtybuffer = gm_zi_malloc (toki_background2_videoram_size / 2)) == 0)
	{
		gm_free (bg1_dirtybuffer);
		gm_free (frg_dirtybuffer);
		return 1;
	}
	...
	gm_memset (frg_dirtybuffer,1,toki_foreground_videoram_size / 2);
	gm_memset (bg2_dirtybuffer,1,toki_background1_videoram_size / 2);
	gm_memset (bg1_dirtybuffer,1,toki_background2_videoram_size / 2);
	return 0;
...
void toki_vh_screenrefresh(struct osd_bitmap *bitmap,int full_refresh)
{
	...
	/* Palette mapping first */
	{
		...
		gm_memset (palette_map, 0, sizeof (palette_map));
		...
		for (palette = 0; palette < 16 * 4; palette++)
		{
			...
			if (usage)
			{
				...
				gm_memset (&palette_used_colors[palette * 16 + 0], PALETTE_COLOR_UNUSED, 16);
		}

		/* recompute */
		if (palette_recalc ())
		{
			gm_memset (frg_dirtybuffer, 1, toki_foreground_videoram_size);
			gm_memset (bg1_dirtybuffer, 1, toki_background1_videoram_size);
			gm_memset (bg2_dirtybuffer, 1, toki_background2_videoram_size);
		}
	}
