...
int superqix_vh_start(void)
{
	...
	if ((paletteram = gm_zi_malloc(256 * sizeof(unsigned char))) == 0)
	{
		generic_vh_stop();
		return 1;
	}

	if ((superqix_bitmapram = gm_zi_malloc(0x7000 * sizeof(unsigned char))) == 0)
	{
		gm_free(paletteram);
		generic_vh_stop();
		return 1;
	...
	if ((superqix_bitmapram2 = gm_zi_malloc(0x7000 * sizeof(unsigned char))) == 0)
	{
		gm_free(superqix_bitmapram);
		gm_free(paletteram);
		generic_vh_stop();
		return 1;
	}

	if ((superqix_bitmapram_dirty = gm_zi_malloc(0x7000 * sizeof(unsigned char))) == 0)
	{
		gm_free(superqix_bitmapram2);
		gm_free(superqix_bitmapram);
		gm_free(paletteram);
		generic_vh_stop();
		return 1;
	}
	gm_memset(superqix_bitmapram_dirty,1,0x7000);

	if ((superqix_bitmapram2_dirty = gm_zi_malloc(0x7000 * sizeof(unsigned char))) == 0)
	{
		gm_free(superqix_bitmapram_dirty);
		gm_free(superqix_bitmapram2);
		gm_free(superqix_bitmapram);
		gm_free(paletteram);
		generic_vh_stop();
	...
	gm_memset(superqix_bitmapram2_dirty,1,0x7000);

	if ((tmpbitmap2 = osd_create_bitmap(256, 256)) == 0)
	{
		gm_free(superqix_bitmapram2_dirty);
		gm_free(superqix_bitmapram_dirty);
		gm_free(superqix_bitmapram2);
		gm_free(superqix_bitmapram);
		gm_free(paletteram);
...
void superqix_0410_w(int offset,int data)
{
	...
	{
		gfxbank = data & 0x03;
		gm_memset(dirtybuffer,1,videoram_size);
	}
