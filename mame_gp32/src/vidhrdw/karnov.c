...
{
	if (dirty_f) gm_free(dirty_f);
	if (karnov_foreground) gm_free(karnov_foreground);
...
int karnov_vh_start (void)
{
	...
	dirty_f=gm_zi_malloc(0x1000);
	karnov_foreground=gm_zi_malloc(0x1000);
	gm_memset(karnov_foreground,0,0x1000);
	gm_memset(dirty_f,1,0x1000);
