...
void swimmer_vh_convert_color_prom(unsigned char *palette, unsigned short *colortable,const unsigned char *color_prom)
{
	...
	gm_memset(palette,0,3 * Machine->drv->total_colors);
...
int cclimber_vh_start(void)
{
	...
	if ((bsdirtybuffer = gm_zi_malloc(cclimber_bsvideoram_size)) == 0)
	{
		generic_vh_stop();
		return 1;
	}
	gm_memset(bsdirtybuffer,1,cclimber_bsvideoram_size);

	if ((bsbitmap = osd_create_bitmap(BIGSPRITE_WIDTH,BIGSPRITE_HEIGHT)) == 0)
	{
		gm_free(bsdirtybuffer);
		generic_vh_stop();
		return 1;
	}
