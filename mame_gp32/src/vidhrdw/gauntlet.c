...
int gauntlet_vh_start(void)
{
	...
	if (!playfielddirty)
		playfielddirty = gm_zi_malloc (atarigen_playfieldram_size / 2);
	if (!playfielddirty)
	{
		gauntlet_vh_stop ();
		return 1;
	}
	gm_memset (playfielddirty, 1, atarigen_playfieldram_size / 2);
	...
	gm_memset (trans_count_col, YCHARS, sizeof (trans_count_col));
	gm_memset (trans_count_row, XCHARS, sizeof (trans_count_row));
...
void gauntlet_vh_screenrefresh(struct osd_bitmap *bitmap,int full_refresh)
{
	...
	{
		if (pf_map[i])
			gm_memset (&palette_used_colors[512 + i * 16], PALETTE_COLOR_USED, 16);
		if (mo_map[i])
		{
		...
		if (al_map[16+i])
			gm_memset (&palette_used_colors[0 + (i+32) * 4], PALETTE_COLOR_USED, 4);
	}
