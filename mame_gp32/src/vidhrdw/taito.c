...
int taito_vh_start(void)
{
	...
	if ((dirtybuffer2 = gm_zi_malloc(videoram_size)) == 0)
	{
		generic_vh_stop();
		return 1;
	}
	gm_memset(dirtybuffer2,1,videoram_size);

	if ((dirtybuffer3 = gm_zi_malloc(videoram_size)) == 0)
	{
		gm_free(dirtybuffer2);
		generic_vh_stop();
		return 1;
	}
	gm_memset(dirtybuffer3,1,videoram_size);
