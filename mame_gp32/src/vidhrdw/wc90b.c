...
int wc90b_vh_start( void ) {
	...
	if ( ( dirtybuffer1 = gm_zi_malloc( wc90b_tile_videoram_size ) ) == 0 ) {
		return 1;
	}

	gm_memset( dirtybuffer1, 1, wc90b_tile_videoram_size );

	if ( ( tmpbitmap1 = osd_new_bitmap(4*Machine->drv->screen_width,2*Machine->drv->screen_height,Machine->scrbitmap->depth ) ) == 0 ){
		gm_free( dirtybuffer1 );
		generic_vh_stop();
		return 1;
	}

	if ( ( dirtybuffer2 = gm_zi_malloc( wc90b_tile_videoram_size2 ) ) == 0 ) {
	...
	gm_memset( dirtybuffer2, 1, wc90b_tile_videoram_size2 );
	...
	/* compute palette usage */
	{
		...
		for (i = 0; i < 1*16; i++)
		{
			...
			if (usage)
			{
				...
				gm_memset (&palette_used_colors[i * 16 + 0], PALETTE_COLOR_UNUSED, 16);
		}
		for (i = 1*16; i < 4*16; i++)
		{
			int usage = palette_map[i], j;
